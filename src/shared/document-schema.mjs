...
const TEXT_FIELDS = Object.freeze({
  ...
  "stay-summary": ["name", "subtitle", "checkin", "checkout", "nights"],
...
function validatePlace(place, ids) {
  ...
  assertTextFields(place, [
    "name", "comment", "mapUrl", "websiteUrl",
...
function validateStayDistances(data) {
  assertText(data.title, "stay distances title", LIMITS.label);
  assertText(data.origin, "stay distances origin", LIMITS.label);
  assertCollection(data.items, "stay distance landmarks", 20);
...
function assertOptionalTextFields(record, fields, label, max = LIMITS.text) {
  for (const field of fields) {
    if (record[field] !== undefined) assertText(record[field], `${label} ${field}`, max);
  }
}
