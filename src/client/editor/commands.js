...
export function changeAmenities(document, context, preset) {
  const block = findBlock(document, context.section, context.blockId);
  if (context.action === "add-group") return block.data.groups.push({ id: `group-${crypto.randomUUID()}`, label: "Nova Categoria", items: [] });
...
export function changeAnatomy(document, context) {
  const block = findBlock(document, context.section, context.blockId);
  if (context.action === "add-space") return block.data.spaces.push({ id: `space-${crypto.randomUUID()}`, label: "Novo Quarto", beds: [] });
  const space = findSpace(document, context);
  if (context.action === "delete-space") return block.data.spaces.splice(findIndex(block.data.spaces, context.spaceId), 1);
  if (context.action === "add-bed") return space.beds.push({ id: `bed-${crypto.randomUUID()}`, label: "Cama de Casal", quantity: 1 });
...
export function addListItem(document, context) {
  const block = findBlock(document, context.section, context.blockId);
  let value = "Novo Item";
  if (block.type === "essentials") value = { label: "Novo Detalhe", value: "Adicionar valor" };
  if (block.type === "stay-distances") value = {
    name: "Novo Ponto de Interesse", address: "Adicionar endereço", drivingDistance: "Adicionar distância", drivingTime: "Adicionar tempo", walkingDistance: "Adicionar distância", walkingTime: "Adicionar tempo", cyclingDistance: "Adicionar distância", cyclingTime: "Adicionar tempo", drivingUrl: "", walkingUrl: "", cyclingUrl: "", cover: null,
  };
...
function addTableColumn(data) { data.columns.push({ id: `column-${crypto.randomUUID()}`, label: `Coluna ${data.columns.length + 1}` }); data.rows.forEach((item) => item.cells.push("")); }
function deleteTableColumn(data) { data.columns.pop(); data.rows.forEach((item) => item.cells.pop()); }
function newGenericItem(type) {
  if (type === "facts") return { id: `item-${crypto.randomUUID()}`, label: "Detalhe", value: "Valor" };
  if (type === "checklist") return { id: `item-${crypto.randomUUID()}`, label: "Nova Tarefa", checked: false };
  return { id: `item-${crypto.randomUUID()}`, label: "Novo Item", iconKey: "check", text: "" };
}
