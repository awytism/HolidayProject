...
function renderPool(section, customTemplates) {
  ...
  return `<div class="template-pool"><button type="button" class="dialog-close" data-template-close aria-label="Close">×</button><small>Block Library</small><h2>Insert a Template</h2><input class="template-search" type="search" placeholder="Search templates" aria-label="Search Templates"><h3>Built-In</h3><div class="template-grid">${builtins}</div><h3>Your Templates</h3><div class="template-grid">${custom || '<p class="template-empty">No custom templates yet.</p>'}</div></div>`;
}

function templateCard(item, kind) {
  return `<button type="button" class="template-card" data-${kind}-template="${escapeHtml(item.id)}">${renderTemplatePrototype(item.type)}<strong>${escapeHtml(item.name)}</strong><span class="template-action">Insert Block</span></button>`;
}
...
function customCard(item) {
  return `<div class="template-card custom"><button type="button" data-custom-template="${escapeHtml(item.id)}"><strong>${escapeHtml(item.name)}</strong><span>Insert Block</span></button><div><button type="button" data-rename-template="${escapeHtml(item.id)}">Rename</button><button type="button" data-delete-template="${escapeHtml(item.id)}">Delete</button></div></div>`;
}
